...
export function Avatar({ bot, size, you, className }: { bot?: Bot; size?: 'sm' | 'xs' | 'lg' | 'xl'; you?: boolean; className?: string }) {
  if (you) return <span className={cx('avatar you', size, className)} title="你"><img src={procedural('you')} alt="你" /></span>;
  ...
  if (bot.generating?.avatar || bot.generating?.identity) return <span className={cx('avatar gen', size, className)} title="生成中…" aria-busy />;
