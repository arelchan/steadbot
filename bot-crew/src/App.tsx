...
export default function App() {
  ...
  return (
    <div className="app no-right" style={{ '--w-sidebar': `${layout.sidebar}px`, '--w-side': `${layout.side}px`, '--w-right': `${layout.right}px` } as React.CSSProperties}>
      <Sidebar />
      {selection === 'week' ? <Week /> : selection === 'inbox' ? <Inbox /> : selection === 'profile' ? <ProfileView /> : selection === 'draft-bot' ? <Draft /> : selection === 'runtime' ? <RuntimeView /> : <Thread threadId={selection as ThreadId} />}
