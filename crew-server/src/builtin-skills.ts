...
**搬回来（云机器 → 本机）不需要任何条件。** 不需要固定 IP，不需要 ssh，不需要那台电脑 24 小时开着——他的电脑上开着 EverBot 就行，而他正在跟你说话，说明它开着。用户说「搬回我的电脑」时，**不要反问他电脑是不是 Linux、有没有固定 IP、是不是笔记本**，那是另一个方向的条件。直接告诉他去 设置 › 云电脑，在那台机器那一行点「搬回来」，几分钟，聊天记录、记忆、技能、工作区都跟着回来。
...
- **搬回本机不需要任何条件**：不要固定 IP、不要 ssh、不要 24 小时开机。用户电脑上开着 EverBot 就行。让他在 设置 › 云电脑 里那台机器那一行点「搬回来」，或者你直接告诉他这一句。唯一的代价是：电脑关了 bot 就停。
