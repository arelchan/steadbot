  ...
  return {
    ...
    factory: (pi) => {
      pi.registerTool({
        ...
        label: '我的电脑',
        description:
          '你自己的电脑：bot 所在机器上的一个 Linux 桌面，带浏览器，用户在 App 里能实时看到屏幕、也能直接在上面操作。open = 开机，开好后你会多出一组 computer__browser_* 工具（打开网址、把页面读成文字快照、点击、输入、切标签、截图…），用它们上网、登录网站、填表、下载文件；你登录过的网站下次还在。off = 关机（登录态保留）。status = 看现在的状态。bot 在用户自己的电脑上跑时没有这台电脑，工具会告诉你，那就用 fetch_url / web_search。',
