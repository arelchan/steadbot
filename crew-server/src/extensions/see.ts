      ...
      pi.registerTool({
        ...
        async execute(_id, p) {
          ...
          if (extname(pdfPath).toLowerCase() === '.pdf') {
            ...
            if (!wantLook && !doc.thin && doc.text.trim().length > 80) return text(await answer(doc.text, p.question, eyes()), 'pdf');
            ...
            try {
              ...
              return text((await look(e, images, p.question, brief)) + note, 'pages');
            } finally {
