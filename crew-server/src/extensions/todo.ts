  ...
  return {
    ...
    factory: (pi) => {
      pi.registerTool({
        ...
        label: '事项',
        description:
          '你的事项本，也是用户在界面上看到的「你在做什么」。用户交给你的每件事都要在这里有一条：新任务 create；有进展、状态变化或用户改了要求 update；做成了 close；不做了（取消、过期、重复、做不了）drop；list 查看。标题写成任务名（如「杭州 9/15 高铁票」），summary 写成一句能读懂的最新进展。',
        promptSnippet: '记录并更新你手上的事项（create / update / close / list）',
        promptGuidelines: [
          '收到用户消息先判断它对事项的影响：新建、更新、做完、不做了，还是只是聊天或提问。只有前四种才调 todo；闲聊、问进展、寒暄不建事项。',
          '一件事只 create 一次；后续变化都是 update。update 的 summary 写「现在到哪一步」，不要重复标题。',
          '事项只有四态：doing 在推进、waiting 不给东西就动不了、done 做成了（close）、closed 不做了（drop）。做完用 close 并在 result 写结果；取消、过期、重复、做不了用 drop 并在 result 写原因——别拿 close 糊弄过去，用户那边这两叠是分开的。',
