  ...
  return {
    ...
    factory: (pi) => {
      ...
      const recall = async () => {
        ...
        return `## 可用但未装\n${hits.map((e) => `- ${e.slug}｜${e.title}：${e.description.slice(0, 80)}`).join('\n')}\n手上的手段做不出像样的东西时，build(action=add, value=slug) 装一个再动手；用不上就当没看见，不用回应。`;
      };
