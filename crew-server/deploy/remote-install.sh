...
set -euo pipefail
target="${1:-}"; domain="${2:-}"
[ -n "$target" ] || { echo "用法：bash crew-server/deploy/remote-install.sh root@服务器IP [域名]" >&2; exit 1; }
here="$(cd "$(dirname "$0")/.." && pwd)"
echo "▸ 同步代码到 ${target}:/opt/crew/crew-server …"
...
echo "▸ 在服务器上安装（第一次要几分钟）…"
ssh -t "$target" "command -v docker >/dev/null 2>&1 || { echo '▸ 先装 Docker …'; curl -fsSL https://get.docker.com | sudo sh; }; sudo DOMAIN='${domain}' CREW_PORT='${CREW_PORT:-5200}' CREW_MTU='${CREW_MTU:-}' bash /opt/crew/crew-server/deploy/install.sh"
