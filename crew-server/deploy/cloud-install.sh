...
[ -n "${DOMAIN:-}" ] || { echo "needs DOMAIN=your.domain (DNS already pointing at this machine)" >&2; exit 1; }
command -v docker >/dev/null || { echo "needs Docker" >&2; exit 1; }
echo "▸ building the tenant image crew-server:local …"; docker build -t crew-server:local .
...
echo; echo "✔ Control plane at https://${DOMAIN}. Build the App with VITE_CREW_CLOUD=https://${DOMAIN}. Admin token: ${admin}"
