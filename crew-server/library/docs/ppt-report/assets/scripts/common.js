...
// ── 自适应：把整张设计稿等比缩放到当前窗口（底部预留 nav 空间） ──
//    设计稿尺寸 / nav 预留 从 CSS 变量读（横屏默认 1600×900×88；竖屏 portrait.css
//    按场景预设覆盖 --design-w/h、--nav-reserve）。一份 fitCanvas 同时服务横竖屏。
function fitCanvas() {
  // 从 <body> 读设计变量：横屏 body 继承 :root 的 1600×900；竖屏 body 上由
  // [data-format="portrait"] / 内联 style 覆盖成 1080×1440 等。从 documentElement(html)
  // 读会拿到 :root 的横屏默认值（竖屏的 body 覆盖影响不到 html）—— 这是个坑。
  const cs = getComputedStyle(document.body);
  const baseW = parseFloat(cs.getPropertyValue('--design-w')) || 1600;
  const baseH = parseFloat(cs.getPropertyValue('--design-h')) || 900;
  const navReserve = parseFloat(cs.getPropertyValue('--nav-reserve'));
  const reserve = isNaN(navReserve) ? 88 : navReserve;
