...
> ✅ **本项目已把这套烘焙进 `assets/scripts/silhouette.js`**(`window.SILHOUETTE`
> 男女剪影 path + viewBox、`window.BODY_ANCHORS` 部位坐标、`window.injectSilhouette()`、
> `window.renderHumanPortrait()`,build 自动注入)。**human-portrait 页直接调
> `renderHumanPortrait()` 即可,无需自己再抽到 common.js**。下面保留方法论是讲原理,
> 成品已内置、不必照着重做。
...
> ✅ **这步本项目已做好,烘焙进 `assets/scripts/silhouette.js`**(`window.SILHOUETTE`
> 男女剪影 path + viewBox、`window.BODY_ANCHORS` 部位坐标、`window.injectSilhouette()`、
> `window.renderHumanPortrait()`,build 自动注入)。human-portrait 页**直接调
> `renderHumanPortrait()` 即可,无需自己再抽**。下面代码示例保留作原理讲解。
